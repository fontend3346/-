...
import { addLIne } from "@/utils/addContour"; //加载轮廓线
...
export const useHandler = (modalEcharts: any) => {
  ...
  const inputChange = (v: any) => {
    ...
    timer.value = setTimeout(() => {
      ...
      measureClick("mapMeasurementSure");
    }, 1000);
  ...
  const stationNetworkChange = (val: any) => {
    ...
    // 定位
    map.flyTo({ center, zoom: state.zoom1 });
  ...
  //台站弹出框 - 点击echarts
   const clickGraph = (item: any) => {
    // netWorkSearchChange()state.statusStation
    if (item.name == "正常") {
  ...
  const stationChildChange = (val: any) => {
    ...
    map.flyTo({ center: [val.longitude, val.latitude], zoom: state.zoom2 });
  };
  //搜索设备tab栏-台站改变事件
  ...
  const equipNetworkChange = (val: any) => {
    ...
    // 定位
    // baseMapObj.flyTo([val.longitude, val.latitude], state.zoom2);
    map.flyTo({ center: [val.longitude, val.latitude], zoom: state.zoom2 }); //飞行到点击位置
  };
  //搜索设备tab栏-查询设备类型事件
  ...
  const selectChangeLayer = (item: any) => {
    if (item.label == "台网") {
      ...
      if (baseMapObj.map.getLayer("GDPTheme")) {
      ...
      if (
        baseMapObj.map.getLayer("provincialline") &&
        baseMapObj.map.getLayer("china")
      ) {
        baseMapObj.chageLayer("provincialline");
        baseMapObj.chageLayer("china");
      } else {
        addTotalLine(); //加载轮廓线
      }
    }
  ...
  // 加载轮廓线
  const addTotalLine = () => {
    addLIne("provincialline").then((res: any) => {
      addContourLine(res, "provincialline"); //加载省级轮廓线
    });
    addLIne("china").then((res: any) => {
      addContourLine(res, "china"); //加载中国轮廓线
    });
  };
  //添加轮廓线
  const addContourLine = (res: any, id: any) => {
    if (id == "china") {
      let option = {
        layerId: id,
        lineColor: "#ffff00",
      ...
      baseMapObj.addLineString(res, option);
    } else if (id == "provincialline") {
      let option = {
        layerId: id,
        lineColor: "#757981",
        dasharray: true,
    ...
    if (
      ...
      baseMapObj.map.moveLayer("provincialline", "china");
    }
  };
  // 正常/异常设备点击
  ...
  const isShowSearch = (isShowNet: Boolean) => {
    //如果它为false显示下面下拉框部分  true显示上面按钮部分
    if (isShowNet) {
      stationAllInfo.stationInfoIshow = true; //搜索台站弹出框
      state.netInfoIsshow = false; //台网弹出框
      baseMapObj.chageLayer("mapInternetSure", true); //台站显示
      ...
      baseMapObj.chageLayer("mapInternetSure", false); //台站隐藏
      addStationsPoint("mapStations"); //台网添加点
      onClickNet("mapStations");
      searchClick();
    }
  ...
  const handleClick = (msg: any) => {
    ...
    state.coverageList.map((item, index) => {
      if (item.layerId != "mapStations") {
        if (msg.index == index) {
          item.value = true;
          if (baseMapObj.map.getLayer(item.layerId)) {
  ...
  const getProvincialline = () => {
    ...
    http.stationsInfoManage.getAddPoint(params).then((res: any) => {
      if (res) {
        ...
        baseMapObj.getBoundary(featureCollection, color);
      } else {
        ElMessage.error(res.message);
      }
    });
  ...
  const thematicMapFn = () => {
    ...
    http.stationsInfoManage.getAddPoint(params).then((res: any) => {
      if (res) {
        ...
        baseMapObj.addPeoplehematic(featureCollection);
      } else {
        ElMessage.error(res.message);
      }
    });
  ...
  const resetFn = () => {
    ...
    // map.flyTo({ center: [116.4, 39.9], zoom: 5 }); //飞行到点击位置
    subjectListChange(state.subjectListList[0].id); //综合台台站
    //查询台站总数
    searchStationNum(state.subjectListList[0].id);
    ...
    baseMapObj.chageLayer("china", false);
  ...
  const clickHeatMap = (obj: any) => {
    ...
    map.flyTo({ center: [obj.longitude, obj.latitude], zoom: 8 }); //飞行到点击位置
