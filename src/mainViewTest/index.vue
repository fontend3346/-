<template>
  <div class="disView">
    <div class="disView-content">
      <!-- 分布展示模块 -->
      <div id="mapContainer" :key="state.random"></div>
      <!-- <PIEMap
        @onload="onload"
        :options="pieOptions"
        style="width: 100%"
      ></PIEMap> -->
      <!-- 工具栏 -->
      <PIEMapTool
        :curTool="state.curTool"
        @checkFn="checkFn"
        @enlargeFn="enlargeFn"
        @reduceFn="reduceFn"
        @checkLayer="checkLayer"
        @coverageList="coverageList"
        @allStatisticsClick="allStatisticsClick"
        @resetFn="resetFn"
        :resetMap="true"
      ></PIEMapTool>
      <!-- 控制图层列表 -->
      <PIECoverage
        class="coverage"
        :pieCoverage="isShowCoverage"
        @selectChange="selectChange"
        :coverageList="state.coverageList"
      ></PIECoverage>
      <!-- 专题图图例 -->
      <PIEThematicLegend
        class="thematic-style"
        :thematicDisplay="state.thematicDisplay"
        :thematicTitle="state.thematicTitle"
        :thematicList="state.thematicList"
        @close="thematicClose"
      ></PIEThematicLegend>
      <!-- 统计 -->
      <div v-if="state.allStatisticsShow" class="table-box">
        <allStatistics @closeFun="closeFun"></allStatistics>
      </div>
      <!-- 底图切换 -->
      <!-- <div class="map-check" v-show="isCheckLayerShow">
        <div
          class="map-check-item"
          v-for="(item, index) in state.baseLayerList"
          :key="index"
        >
          <img :src="item.img" @click="checkMapFn(item)" />
        </div>
      </div> -->
      <div class="map-check" v-show="isCheckLayerShow">
        <div class="map-check-item" @click="checkMapFn(1)">
          <img src="@/assets/images/map1.png" alt="" />
        </div>
        <div class="map-check-item" @click="checkMapFn(2)">
          <img src="@/assets/images/map2.png" alt="" />
        </div>
      </div>
      <!-- 图例展开与收起 -->
      <el-tooltip
        class="box-item"
        effect="dark"
        content="图例"
        placement="top"
        v-if="!showlegend && state.compatibleLegend"
      >
        <div
          :class="['legend-icon', 'iconfont', 'legend-icon-up']"
          @click="clicklegend"
        ></div>
        <!-- 'icon-daohangshouqi'  -->
      </el-tooltip>
      <!-- 图例 -->
      <div class="legend-style" v-if="showlegend">
        <PIELegend
          @close="closeLegend"
          iconclass="icon-daohangshouqi"
        ></PIELegend>
      </div>
      <!-- 台站点击-弹框 -->
      <!-- v-model:visible="showPonit" -->
      <PIEPanel
        class="pie-equip-panel"
        v-model:visible="showPonit"
        :title="markData.pointTitle"
        :width="21.7"
        :height="40"
        :cancelBtn="false"
        @cancel="cancelStation"
        :confirmBtn="false"
        :moveLeft="'73.5%'"
        :moveTop="'9.5%'"
      >
        <template v-slot:operateItem>
          <div class="ponit-modal">
            <!-- 正常/异常设备 -->
            <div class="point-instrument"></div>
            <div class="point-modal-echart-box">
              <div class="echart-title">设备总览</div>
              <div class="point-modal-echarts" ref="modalEcharts"></div>
            </div>
            <div class="point-modal-center">
              <div class="title">台站名称：</div>
              <div class="info">{{ markData.detailPoint.name }}</div>
            </div>
            <div class="point-modal-center">
              <div class="title">台站编码：</div>
              <div class="info">{{ markData.detailPoint.code }}</div>
            </div>
            <div class="point-modal-center">
              <div class="title">台阵类型：</div>
              <div class="info">{{ markData.detailPoint.stationtypename }}</div>
            </div>
            <div class="point-modal-center">
              <div class="title">隶属台网：</div>
              <div class="info">{{ markData.detailPoint.networkname }}</div>
            </div>
            <div class="point-modal-center">
              <div class="title">经纬度：</div>
              <div class="info">
                {{ markData.detailPoint.longitude }},{{
                  markData.detailPoint.latitude
                }}
              </div>
            </div>
            <!-- <div class="point-modal-center">
              <div class="title">创建时间：</div>
              <div class="info">{{ markData.detailPoint.create_time }}</div>
            </div>
            <div class="point-modal-center">
              <div class="title">高程：</div>
              <div class="info">{{ markData.detailPoint.elevation }}</div>
            </div> -->
            <!-- <div class="point-modal-center">
              <div class="title">台站岩性：</div>
              <div class="info">
                {{ markData.detailPoint.stationLithology }}
              </div>
            </div> -->
            <!-- <div class="point-modal-center">
              <div class="title">详细地址：</div>
              <div class="info">{{ markData.detailPoint.stationAddress }}</div>
            </div>
            <div class="point-modal-center">
              <div class="title">管理方式：</div>
              <div class="info">
                {{ markData.detailPoint.staionSupervisorMode }}
              </div>
            </div>
            <div class="point-modal-center">
              <div class="title">台站级别：</div>
              <div class="info">
                {{ markData.detailPoint.stationPropertyStr }}
              </div>
            </div> -->
            <!--  v-show="state.isMonitor" -->
            <div class="point-modal-center">
              <div class="band-button" @click="bandClick">台站监控</div>
              <div class="band-button" @click="modelClick">台站模型</div>
            </div>
            <PIETable
              :maxHeight="200"
              :data="facilityInfo.linkageData"
              :columns="facilityInfo.linkageColumns"
              :isAction="true"
              :detailBtn="true"
              @detail="look"
              :operationWidth="120"
              :header-cell-style="{
                background: 'rgba(95, 152, 255, 0.1)',
                fontSize: '14px',
                height: '50px',
                width: '80px',
                fontWeight: '200',
                letterSpacing: '3px',
              }"
              :cell-style="{
                background: 'transparent',
                fontSize: '14px',
                height: '50px',
                width: '80px',
                fontWeight: '200',
                color: '@main-font-color',
                letterSpacing: '2px',
              }"
              :row-style="{
                borderBottom: '1px solid #13598C',
                background: 'transparent',
              }"
            >
              <template v-slot:status="status">
                <!-- （0正常，1故障，2预警，3离线） -->
                <div v-if="status.row.status == 0" class="status-available">
                  <span class="iconfont icon-yuandianda"></span>
                </div>
                <div v-if="status.row.status == 1" class="status-failure">
                  <span class="iconfont icon-yuandianda"></span>
                </div>

                <div v-if="status.row.status == 2" class="status-warning">
                  <span class="iconfont icon-yuandianda"></span>
                </div>
                <div v-if="status.row.status == 3" class="status-unavailable">
                  <span class="iconfont icon-yuandianda"></span>
                </div>
              </template>
              <template #action="scope">
                <span class="lookBtn-table" @click="waveaction(scope.row)"
                  >波形</span
                >
              </template>
            </PIETable>
            <PIEPage
              :total="facilityInfo.total"
              :pageSize="state.pageSize"
              :currentPage="facilityInfo.pageNum"
              @handleSizeChange="sizeChangeDevice"
              @handleNumChange="pageChanges"
              :isSmall="'prev, pager, next'"
              :smallSize="true"
            ></PIEPage>
          </div>
        </template>
      </PIEPanel>
      <!-- 搜索 - 设备弹出框 -->
      <PIEPanel
        class="pie-equip-panel"
        v-model:visible="equipInfo.ShowPonit"
        :title="markData.pointTitle"
        :width="21.7"
        :height="40"
        :cancelBtn="false"
        @cancel="cancelStation"
        :confirmBtn="false"
        :moveLeft="'73.5%'"
        :moveTop="'9.5%'"
      >
        <template v-slot:operateItem>
          <div class="ponit-modal">
            <PIETable
              :maxHeight="200"
              :data="facilityInfo.linkageData"
              :columns="facilityInfo.linkageColumns"
              :isAction="true"
              :lookBtn="true"
              @look="look"
              :operationWidth="120"
              :header-cell-style="{
                background: 'rgba(95, 152, 255, 0.1)',
                fontSize: '14px',
                height: '50px',
                width: '80px',
                fontWeight: '200',
                letterSpacing: '3px',
              }"
              :cell-style="{
                background: 'transparent',
                fontSize: '14px',
                height: '50px',
                width: '80px',
                fontWeight: '200',
                color: '@main-font-color',
                letterSpacing: '2px',
              }"
              :row-style="{
                borderBottom: '1px solid #13598C',
                background: 'transparent',
              }"
            >
              <template v-slot:status="status">
                <!-- （0正常，1故障，2预警，3离线） -->
                <div v-if="status.row.status == 0" class="status-available">
                  <span class="iconfont icon-yuandianda"></span>
                </div>
                <div v-if="status.row.status == 1" class="status-failure">
                  <span class="iconfont icon-yuandianda"></span>
                </div>

                <div v-if="status.row.status == 2" class="status-warning">
                  <span class="iconfont icon-yuandianda"></span>
                </div>
                <div v-if="status.row.status == 3" class="status-unavailable">
                  <span class="iconfont icon-yuandianda"></span>
                </div>
              </template>
              <template #action="scope">
                <span class="lookBtn-table" @click="waveaction(scope.row)"
                  >波形</span
                >
              </template>
            </PIETable>
            <PIEPage
              :total="facilityInfo.total"
              :pageSize="state.pageSize"
              :currentPage="facilityInfo.pageNum"
              @handleSizeChange="sizeChangeDevice"
              @handleNumChange="pageChanges"
              :isSmall="'prev, pager, next'"
              :smallSize="true"
            ></PIEPage>
          </div>
        </template>
      </PIEPanel>
      <!-- 台站详情>台站监控  -->
      <PIEFacility
        :showInstrument="showInstrument"
        :facilityId="facilityId"
        :equipmentArr="facilityInfo.equipmentArr"
        :isShowFlage="equipInfo.isShowFlage"
        @calcelInstrument="calcelInstrument"
        @equipmentChange="equipmentChange"
        @bandConfirm="bandConfirm"
        @echartDragOne="echartDragOne"
        @historyWeave="historyWeave"
        @historyCancel="historyCancel"
        :width="90"
        :height="70"
        moveLeft="4.5%"
        moveTop="15%"
      ></PIEFacility>
      <!-- <PIEFacility
        :showInstrument="showInstrument"
        :equipmentArr="facilityInfo.equipmentArr"
        :width="80"
        :height="70"
        moveLeft="10%"
        moveTop="15%"
      ></PIEFacility> -->
      <!--台站详情>正常/异常设备点击弹窗-->
      <PIEPanel
        class="facility-table"
        v-model:visible="facility"
        title="设备"
        :width="27"
        :height="30"
        :cancelBtn="false"
        :confirmBtn="false"
        :moveLeft="'42%'"
        :moveTop="'20%'"
      >
        <template v-slot:operateItem>
          <!-- 设备信息 -->
          <div class="facility-Info">
            <PIETable
              :maxHeight="200"
              :data="facilityInfo.linkageData"
              :columns="facilityInfo.linkageColumns"
              :showIndex="true"
              :operationWidth="130"
              :header-cell-style="{
                background: 'rgba(95, 152, 255, 0.1)',
                fontSize: '16px',
                height: '50px',
                fontWeight: '200',
                letterSpacing: '3px',
              }"
              :cell-style="{
                background: 'transparent',
                fontSize: '15px',
                height: '50px',
                fontWeight: '200',
                color: '@main-font-color',
                letterSpacing: '2px',
              }"
              :row-style="{
                borderBottom: '1px solid #13598C',
                background: 'transparent',
              }"
            >
            </PIETable>
          </div>
        </template>
      </PIEPanel>
      <!--波段确定展示大图 “保存样本”框-->
      <PIEPanel
        class="panel-instrument-big"
        v-model:visible="showBigImgEcharts"
        title="保存事件"
        :width="25"
        :height="20"
        :cancelBtn="false"
        :confirmBtn="false"
        :moveLeft="'35%'"
        :moveTop="'30%'"
      >
        <template v-slot:operateItem>
          <div class="big-echarts">
            <div class="sample-name">
              <span>事件名称</span>
              <el-input
                v-model="sampleValue"
                placeholder="请输入事件名称"
              ></el-input>
            </div>
            <div class="sample-name">
              <span class="select-style-name">事件类型:</span>
              <el-select
                class="select-style"
                v-model="eventType"
                placeholder="请选择事件类型"
                clearable
              >
                <el-option
                  v-for="item in state.installPerList"
                  :key="item.label"
                  :label="item.label"
                  :value="item.value"
                />
              </el-select>
            </div>
            <div class="sample-upload">
              <el-button @click="sampleClick(1)">保存</el-button>
            </div>
          </div>
        </template>
      </PIEPanel>
      <!--波段确定展示大图 “保存样本”框 从设备弹框点入-->
      <PIEPanel
        class="panel-instrument-big"
        v-model:visible="showBigImgEchartsEquip"
        title="保存样本"
        :width="25"
        :height="20"
        :cancelBtn="false"
        :confirmBtn="false"
        :moveLeft="'38%'"
        :moveTop="'40%'"
      >
        <template v-slot:operateItem>
          <div class="big-echarts">
            <div class="sample-name">
              <span>样本名称</span>
              <el-input
                v-model="sampleValue"
                placeholder="请输入样本名称"
              ></el-input>
            </div>
            <div class="sample-upload">
              <el-button @click="sampleClick(2)">保存</el-button>
            </div>
          </div>
        </template>
      </PIEPanel>
      <!-- 地图弹框（台网） -->
      <PIEMapPointDetails
        class="isNetwork"
        ref="modalRef"
        v-show="markShow"
        :valueData="state.markData1"
        :valueDataElse="state.valueDataElse"
        v-model:showInfo="showInfoMark"
        :showUpdate="showUpdate"
        :isConfig="false"
        :tableShow="isShowFacility"
        close="true"
        :title="netWorkTitle"
        @cancel="cancel"
        @facilityNetworkFun="facilityNetMark"
      >
        <template v-slot:tableItem>
          <!-- 设备、台网、测点信息 -->
          <div class="facility-Info">
            <PIETable
              height="200"
              :maxHeight="200"
              :data="facilityNetwork.linkageData"
              :columns="facilityNetwork.linkageColumns"
              :showIndex="true"
              :operationWidth="130"
              :header-cell-style="{
                background: 'rgba(95, 152, 255, 0.1)',
                fontSize: '16px',
                height: '50px',
                fontWeight: '200',
                letterSpacing: '3px',
              }"
              :cell-style="{
                background: 'transparent',
                fontSize: '15px',
                height: '50px',
                fontWeight: '200',
                color: '@main-font-color',
                letterSpacing: '2px',
              }"
              :row-style="{
                borderBottom: '1px solid #13598C',
                background: 'transparent',
              }"
              @on-row-click="rowclick"
            >
            </PIETable>
            <div class="pie-tb-footer">
              <PIEPage
                :total="state.total"
                :pageSize="state.pageSize"
                :currentPage="state.pageNum"
                @handleSizeChange="sizeChangeDevice"
                @handleNumChange="pageChangeDevice"
                :isSmall="'prev, pager, next'"
                :smallSize="true"
              ></PIEPage>
            </div>
          </div>
        </template>
      </PIEMapPointDetails>
      <!-- 点击台网弹框，点击“设备”出现的弹框 -->
      <!-- <PIEPanel
        class="facility-table"
        v-model:visible="state.netDataShow"
        :title="facilityNetwork.title"
        :width="27"
        :height="30"
        :cancelBtn="false"
        :confirmBtn="false"
        :moveLeft="'42%'"
        :moveTop="'21%'"
      >
        <template v-slot:operateItem>
        </template>
      </PIEPanel> -->
      <!-- 概览弹窗  v-model:visible="state.netInfoIsshow-->
      <div class="net-box" v-show="state.netInfoIsshow">
        <div class="title-line">
          <div class="iconfont icon-liebiao"></div>
          <span>{{ state.netWorkTitle }}概览</span>
          <div
            class="iconfont icon-guanbi cancel-icon"
            @click="overviewNet"
          ></div>
        </div>
        <div class="echarts-info-title">
          <div class="echarts-info">
            <PIEGraph
              class="echarts-item left-item"
              :subtext="state.stationStatusInfo.subtext"
              :data="state.stationStatusInfo.data"
              :title="'台站总览'"
            ></PIEGraph>
            <PIEGraph
              class="echarts-item"
              :subtext="state.equipmentStatusInfo.subtext"
              :data="state.equipmentStatusInfo.data"
              :title="'设备总览'"
            >
            </PIEGraph>
          </div>
        </div>

        <!-- <PIENetInfo :equipmentList="state.stationClassify"></PIENetInfo>
      <div class="histogram-group">
        <div class="histogram-title">
          {{ histogramProps.title }}设备正确率Top10
        </div>
        <PIEHistogram
          class="histogram"
          :xAxisData="histogramProps.xAxisData"
          :yAxisData="histogramProps.yAxisData"
        ></PIEHistogram> -->

        <PIENetInfo
          :equipmentList="state.stationClassify"
          @clickSubjectbox="clickSubjectbox"
        ></PIENetInfo>
        <div class="histogram-group">
          <div class="histogram-title">{{ histogramProps.title }}</div>
          <!-- 点击事件修改热力图与关系图 -->
          <!-- <PIEHistogram
          class="histogram"
          :xAxisData="histogramProps.xAxisData"
          :yAxisData="histogramProps.yAxisData"
          v-if="histogramProps.title != '台网'"
        ></PIEHistogram> -->
          <PIEDiagram
            class="histogram"
            v-if="histogramProps.title != '台网正常率'"
            :nodes="diagramProps.nodes"
            :links="diagramProps.links"
            :fresh="diagramProps.fresh"
            :key="new Date().getTime()"
            @clickNode="clickDiagram"
          ></PIEDiagram>
          <PIEHeatMap
            class="histogram"
            :data="state.heatMapData"
            v-if="histogramProps.title == '台网正常率'"
            :key="new Date().getTime()"
            @clickHeatMap="clickHeatMap"
          ></PIEHeatMap>
        </div>
      </div>
      <!-- 点击测点弹框 -->
      <PIEThreeMeasure
        :measureShow="measureShow"
        :measureObj="measureObj"
        :stationInfo="state.stationInfo"
        :netInfo="state.netInfo"
        @cancel="cancel"
      ></PIEThreeMeasure>
      <!-- 设备详情 -->
      <PIEPanel
        v-model:visible="state.equipmentDetails"
        title="设备详情"
        :width="45"
        :height="75"
        :cancelBtn="false"
        :confirmBtn="false"
        :moveLeft="'28%'"
        :moveTop="'6%'"
        class="quipment-details"
      >
        <template v-slot:operateItem>
          <div class="modal-wrapper">
            <!-- 基本信息 -->
            <div class="essential-information">
              <div class="information-title">基本信息</div>
              <div class="info-item">
                <div class="title">经纬度 :</div>
                <div>
                  {{ state.equipmentDetailsInfo.Longitude }},{{
                    state.equipmentDetailsInfo.Latitude
                  }}
                </div>
              </div>
              <div class="info-item">
                <div class="title">高度 :</div>
                <div>{{ state.equipmentDetailsInfo.Altitude }}</div>
              </div>
              <div class="info-item">
                <div class="title">GPS(h) :</div>
                <div>
                  {{ state.equipmentDetailsInfo["GPS(h)"]
                  }}<span
                    class="iconfont icon-shishijiance monitor-img"
                    @click="detailImg('G')"
                  ></span>
                </div>
              </div>
              <div class="info-item">
                <div class="title">IP地址 :</div>
                <div>{{ state.equipmentDetailsInfo["IP"] }}</div>
              </div>
              <PIETable
                :maxHeight="130"
                :data="state.equipmentDetailsInfo.networkData"
                :columns="state.networkColumns"
                :header-cell-style="{
                  background: 'rgba(95, 152, 255, 0.1)',
                  fontSize: '14px',
                  height: '40px',
                  fontWeight: '200',
                  letterSpacing: '3px',
                }"
                :cell-style="{
                  background: 'transparent',
                  fontSize: '13px',
                  height: '40px',
                  fontWeight: '200',
                  color: '@main-font-color',
                  letterSpacing: '2px',
                }"
                :row-style="{
                  borderBottom: '1px solid #13598C',
                  background: 'transparent',
                }"
                class="table-content"
              >
              </PIETable>
            </div>
            <!-- 电压详情 -->
            <div class="essential-information">
              <div class="information-title">
                电压详情<span
                  class="iconfont icon-shishijiance monitor-img"
                  @click="detailImg('T')"
                ></span>
              </div>
              <div class="info-item">
                <div class="title">是否采集 :</div>
                <div>{{ state.equipmentDetailsInfo["Acquisition"] }}</div>
              </div>
              <div class="info-item">
                <div class="title">数采设备温度 :</div>
                <div>{{ state.equipmentDetailsInfo["Temp."] }}</div>
              </div>
              <div class="info-item">
                <div class="title">输入电压 :</div>
                <div>{{ state.equipmentDetailsInfo["Input"] }}</div>
              </div>
              <div class="info-item">
                <div class="title">备份电压 :</div>
                <div>{{ state.equipmentDetailsInfo["Backup"] }}</div>
              </div>
            </div>
            <!-- 内存详情 -->
            <div class="essential-information">
              <div class="information-title">
                内存详情<span
                  class="iconfont icon-shishijiance monitor-img"
                  @click="detailImg('R')"
                ></span>
              </div>
              <div class="info-item">
                <div class="title">延迟(s) :</div>
                <div>{{ state.equipmentDetailsInfo["Delay(s)"] }}</div>
              </div>
              <div class="info-item">
                <div class="title">内存(Kb) :</div>
                <div>{{ state.equipmentDetailsInfo["RAM(Kb)"] }}</div>
              </div>
              <div class="info-item">
                <div class="title">本地磁盘使用率 :</div>
                <div>{{ state.equipmentDetailsInfo["Disk1(Mb)"] }}</div>
              </div>
              <div class="info-item">
                <div class="title">本地磁盘使用率2 :</div>
                <div>{{ state.equipmentDetailsInfo["Disk2(Mb)"] }}</div>
              </div>
            </div>
            <!-- 通道参数 -->
            <div class="table-content">
              <!-- <div class="information-title">通道参数</div> -->
              <PIETable
                :maxHeight="150"
                :data="state.equipmentDetailsInfo.channelData"
                :columns="state.channelColumns"
                :header-cell-style="{
                  background: 'rgba(95, 152, 255, 0.1)',
                  fontSize: '14px',
                  height: '40px',
                  fontWeight: '200',
                  letterSpacing: '3px',
                }"
                :cell-style="{
                  background: 'transparent',
                  fontSize: '13px',
                  height: '40px',
                  fontWeight: '200',
                  color: '@main-font-color',
                  letterSpacing: '2px',
                }"
                :row-style="{
                  borderBottom: '1px solid #13598C',
                  background: 'transparent',
                }"
              >
              </PIETable>
            </div>
            <!-- 数据流参数 -->
            <div class="table-content">
              <!-- <div class="information-title">数据流参数</div> -->
              <PIETable
                :maxHeight="150"
                :data="state.equipmentDetailsInfo.streamData"
                :columns="state.streamColumns"
                :header-cell-style="{
                  background: 'rgba(95, 152, 255, 0.1)',
                  fontSize: '14px',
                  height: '40px',
                  fontWeight: '200',
                  letterSpacing: '3px',
                }"
                :cell-style="{
                  background: 'transparent',
                  fontSize: '13px',
                  height: '40px',
                  fontWeight: '200',
                  color: '@main-font-color',
                  letterSpacing: '2px',
                }"
                :row-style="{
                  borderBottom: '1px solid #13598C',
                  background: 'transparent',
                }"
              >
              </PIETable>
            </div>
            <!-- 状态参数 -->
            <div>
              <!-- <div class="information-title">数据流参数</div> -->
              <PIETable
                :maxHeight="150"
                :data="state.equipmentDetailsInfo.statusData"
                :columns="state.statusColumns"
                :header-cell-style="{
                  background: 'rgba(95, 152, 255, 0.1)',
                  fontSize: '14px',
                  height: '40px',
                  fontWeight: '200',
                  letterSpacing: '3px',
                }"
                :cell-style="{
                  background: 'transparent',
                  fontSize: '13px',
                  height: '40px',
                  fontWeight: '200',
                  color: '@main-font-color',
                  letterSpacing: '2px',
                }"
                :row-style="{
                  borderBottom: '1px solid #13598C',
                  background: 'transparent',
                }"
              >
              </PIETable>
            </div>
          </div>
        </template>
      </PIEPanel>
      <!-- 设备详情-设备类型为 供电设备 -->
      <PIEModal
        width="50"
        height="50"
        title="设备详情"
        v-model:visible="state.showEquipmentDetail"
        :close="true"
        :showBtn="false"
      >
        <template #operateItem>
          <div class="modal-wrapper">
            <div
              class="modal-content modal-margin"
              v-for="(item, index) in state.powerEquipmentDetails"
              :key="index"
            >
              <span class="equdetails">{{ index }}</span>

              <span class="equdetails">{{ item }}</span>
            </div>
            <!-- <div class="modal-content">
                <span class="equdetails">通道信息</span>
                <div class="look-style" @click="look">点此查看</div>
              </div> -->
          </div>
        </template>
      </PIEModal>
      <!-- 设备详情信息图片 -->
      <PIEPanel
        v-model:visible="state.equipDetailsImgShow"
        title="预览"
        :width="90"
        :height="60"
        :cancelBtn="false"
        :confirmBtn="false"
        :moveLeft="'5%'"
        :moveTop="'8%'"
        class="quipment-details"
      >
        <template v-slot:operateItem>
          <div class="img-content">
            <div v-for="(item, index) in state.equipDetailsImg" :key="index">
              <img class="detail-img" :src="item" alt="" />
            </div>
          </div> </template
      ></PIEPanel>
      <!-- 搜索台站弹出框 -->
      <div class="net-box" v-show="stationAllInfo.stationInfoIshow">
        <div class="title-line">
          <div class="iconfont icon-liebiao"></div>
          <span v-show="stationAllInfo.stationTitleSub"
            >{{ stationAllInfo.stationTitleSub + "学科" }}
          </span>
          <span v-show="stationAllInfo.stationTitleType">{{
            stationAllInfo.stationTitleType
          }}</span>
          <span>概览</span>
          <div
            class="iconfont icon-guanbi cancel-icon"
            @click="stationBoxClose"
          ></div>
        </div>
        <div class="echarts-info-title">
          <div class="echarts-info">
            <PIEGraph
              class="echarts-item left-item"
              :subtext="stationAllInfo.stationStatusInfo.subtext"
              :data="stationAllInfo.stationStatusInfo.data"
              :title="'台站总览'"
              @clickGraph="clickGraph"
            ></PIEGraph>
            <PIEGraph
              class="echarts-item"
              :subtext="stationAllInfo.equipmentStatusInfo.subtext"
              :data="stationAllInfo.equipmentStatusInfo.data"
              :title="'设备总览'"
            >
            </PIEGraph>
          </div>
        </div>
        <div class="station-top-item">
          <div class="station-title">台网</div>
          <el-select
            v-model="state.netWorkSearch"
            clearable
            filterable
            placeholder="请选择设备类型"
            @change="networkIdVal"
          >
            <el-option
              v-for="item in state.netWorkListSearch"
             :key="item.networkId"
              :label="item.cname"
              :value="item.networkId"
            >
            </el-option>
          </el-select>
        </div>
        <div class="table-style">
          <PIETable
            :maxHeight="300"
            :data="stationAllInfo.linkageData"
            :columns="stationAllInfo.linkageColumns"
            :showIndex="true"
            @on-row-click="stationRowclick"
            :header-cell-style="{
              background: 'rgba(95, 152, 255, 0.1)',
              fontSize: '16px',
              height: '50px',
              fontWeight: '200',
              letterSpacing: '3px',
            }"
            :cell-style="{
              background: 'transparent',
              fontSize: '15px',
              height: '50px',
              fontWeight: '200',
              color: '@main-font-color',
              letterSpacing: '2px',
            }"
            :row-style="{
              borderBottom: '1px solid #13598C',
              background: 'transparent',
            }"
          >
          </PIETable>
          <div class="pie-tb-footer">
            <PIEPage
              :total="stationAllInfo.total"
              :pageSize="stationAllInfo.pageSize"
              :currentPage="stationAllInfo.pageNum"
              @handleSizeChange="sizeChangeSearch"
              @handleNumChange="pageChangeSearch"
              :isSmall="'prev, pager, next'"
              :smallSize="true"
            ></PIEPage>
          </div>
        </div>
      </div>
      <!-- 全部查询 -->
      <PIESelect
        v-if="state.facility"
        class="search-style"
        :stationTotalInfo="state.stationTotalInfo"
        :roleLevel="state.roleLevel"
        :selectNetList="state.selectNetList"
        :selectNetLists="state.selectNetLists"
        :foreignList="state.foreignList"
        :stationChildList="state.stationChildList"
        :subjectListList="state.subjectListList"
        :stationTypeAll="state.stationTypeAll"
        :stationChildList2="state.stationChildList2"
        :deviceList="state.deviceList"
        :deviceName="deviceName"
        :stationTableShow="stationTableShow"
        :stationTableList="facilityNetwork.stationTableList"
        :stationColumns="facilityNetwork.stationColumns"
        :seeList="facilityNetwork.seeList"
        :seeColumns="facilityNetwork.seeColumns"
        :activeName="activeName"
        :subjectTypeList="state.subjectTypeList"
        :equipmentTypeList="state.equipmentTypeList"
        @TestSelect="TestSelect"
        @cutFun="cutFun"
        @subjectTypeClick="subjectTypeClick"
        @cancelSelect="cancelSelect"
        @handleClick="handleClick"
        @networkChange1="networkChange1"
        @stationNetworkChange="stationNetworkChange"
        @subjectTypeChange="subjectTypeChange"
        @netCodeChange="netCodeChange"
        @networkChange="networkChange"
        @stationChildChange="stationChildChange"
        @subjectListChange="subjectListChange"
        @stationEquipChange="stationEquipChange"
        @equipNetworkChange="equipNetworkChange"
        @stationCodeChange="stationCodeChange"
        @queryDeviceName="queryDeviceName"
        @deviceCodeChange="deviceCodeChange"
        @stationSelect="stationSelect"
        @foreignChang="foreignChang"
        @equipmentTypeChange="equipmentTypeChange"
        @statinoTypeClick="statinoTypeClick"
        @isShowSearch="isShowSearch"
      ></PIESelect>
    </div>
    <div class="select-btn" @click="searchClick" v-show="state.isShowInput">
      <el-input placeholder="请输入关键词">
        <template #prepend>
          <img class="search-img" src="@/assets/image/select.png" alt="搜索" />
        </template>
      </el-input>
    </div>
  </div>
</template>

<script lang="ts" setup>
import allStatistics from "./allStatistics.vue";
import { useHandler } from "./index";
import { useHandlerThree } from "./threeSystem/threeMeasure";
import { reactive, nextTick, onMounted, ref, watch } from "vue";
let threeRef: any = ref(null);
let measurShow = ref(false);
let modalEcharts: any = ref(null);
const {
  state,
  networkIdVal,
  clickGraph,
  netWorkSearchChange,
  overviewNet,
  stationBoxClose,
  cutFun,
  subjectTypeClick,
  statinoTypeClick,
  isShowSearch,
  stationRowclick,
  stationAllInfo,
  sizeChangeSearch,
  pageChangeSearch,
  equipmentTypeChange,
  showTable,
  tableNumber,
  activeName,
  isTableShow,
  isSpecilShow,
  echartsData,
  markShow,
  showInfoMark,
  netWorkTitle,
  showUpdate,
  modalRef,
  isShowFacility,
  stationTableShow,
  seePointShow,
  facilityNetwork,
  equipTitle,
  markShowEquip,
  cancelModalEquip,
  showInfoMarkEquip,
  showUpdateEquip,
  modalRefEquip,
  deviceName,
  equipmentType,
  equipmentCode,
  popup,
  isCheckLayerShow,
  searchVal,
  markData,
  showPonit,
  cancelStation,
  showInstrument,
  calcelInstrument,
  facility,
  sampleValue,
  eventType,
  equipmentVal,
  showBigImgEcharts,
  showBigImgEchartsEquip,
  facilityInfo,
  detailId,
  initMap,
  getProvince,
  getTaiwanese,
  getStations,
  getCountry,
  getTreeStations,
  baseMapObj,
  map,
  samllBaseMapObj,
  smallMap,
  options1,
  waveOptionList,
  options2,
  options3,
  options4,
  getWorker,
  work,
  getWorker2,
  cancel,
  facilityNetMark,
  toTable,
  closeTable,
  isShowCoverage,
  timer,
  inputChange,
  addStationsPoint,
  addInternetPoint,
  internetApi,
  addMeasurementPoint,
  measurementApi,
  addEquipmentPoint,
  treeProps,
  onClicks,
  onClickNet,
  onClicksEquip,
  updateInfo,
  submitForm,
  delInfo,
  searchId,
  provinceTaiwanese,
  stationsTotal,
  foreignChang,
  networkChange,
  networkChange1,
  stationNetworkChange,
  subjectTypeChange,
  netCodeChange,
  addStationPoints,
  flyProvince,
  stationArrayChange,
  stationArrayChange1,
  stationChildChange,
  subjectListChange,
  stationEquipChange,
  equipNetworkChange,
  stationCodeChange,
  addStationChildPoints,
  addPointNormal,
  checkFn,
  enlargeFn,
  reduceFn,
  checkLayer,
  coverageList,
  allStatisticsClick,
  closeFun,
  selectChange,
  thematicClose,
  selectChangeLayer,
  checkMapFn,
  instrumentClick,
  pageChange,
  pageChanges,
  sizeChange,
  searchList,
  searchFacilityNum,
  showEchartsValue,
  showWave,
  showWaveBottom,
  bandClick,
  listenCount,
  echartDragOne,
  echartDragTwo,
  echartDragThree,
  echartDragFour,
  bandConfirm,
  bandConfirmEquip,
  equipmentChange,
  sampleClick,
  searchClick,
  cancelSelect,
  nodeClick,
  displayNet,
  displayStation,
  treeShowClick,
  contourLines,
  equipmentNameQuery,
  queryDeviceName,
  deviceCodeChange,
  handleClick,
  TestSelect,
  stationSelect,
  measureShow,
  measureObj,
  sizeChangeDevice,
  pageChangeDevice,
  closeLegend,
  pieOptions,
  onload,
  showlegend,
  clicklegend,
  rowclick,
  look,
  waveaction,
  histogramProps,
  modelClick,
  diagramProps,
  clickDiagram,
  detailImg,
  equipInfo,
  clickSubjectbox,
  resetFn,
  clickHeatMap,
  facilityId,
  historyWeave,
  historyCancel,
} = useHandler(modalEcharts);
// 测点模型
watch(measureShow, () => {
  if (measureShow.value) {
    nextTick(() => {
      initModel();
    });
  }
});

const { initModel, moveShow, treePanelRef, threeData } =
  useHandlerThree(threeRef);
</script>

<style lang="less" scoped>
//覆盖表格
.table-box {
  position: fixed;
  // z-index: 9999;
  z-index: 4;
  top: 5%;
  left: 0%;
  width: 100%;
  height: 95%;
  // padding-top: 40px;
  // padding-left: 40px;
  background: rgba(10, 37, 88, 1);
  display: flex;
  .iconfont {
    position: absolute;
    right: 1%;
    color: #fff;
    top: 2%;
    padding: 5px;
    border: 1px solid #fff;
    border-radius: 50%;
    z-index: 999;
  }
}
.disView {
  height: 100%;
  width: 100%;
  // position: relative;
}
.disView-content {
  height: 100%;
  position: relative;
  overflow: hidden;
}
.coverage {
  position: absolute;
  right: 60px;
  top: 300px;
  // width: 100px;
  // height: 100px;
  border: 1px solid #05c1f5;
  background: rgba(14, 35, 90, 0.5);
  z-index: 10;
}
// 专题图图例
.thematic-style {
  position: absolute;
  left: 43px;
  bottom: 10px;
  // width: 100px;
  // height: 100px;
  border: 1px solid #05c1f5;
  background: rgba(14, 35, 90, 0.5);
  z-index: 10;
}
#mapContainer {
  width: 100%;
  height: 100%;
  // position: relative;
  :deep(.local-boxs) {
    .mapboxgl-popup-content {
      padding: 0;
      background-color: transparent;
    }

    .mapboxgl-popup-close-button {
      color: #efe93b !important;
      font-size: 24px;
      right: 24px;
      top: 6px;
    }

    .mapboxgl-popup-anchor-left .mapboxgl-popup-tip {
      background-color: transparent;
      border-right-color: transparent;
    }
  }
}
.map-tool {
  position: absolute;
  top: 100px;
  // bottom: 100px;
  right: 20px;
  text-align: center;
  padding: 5px;
  box-sizing: border-box;
  z-index: 10;
  .tool-item {
    width: 30px;
    height: 30px;
    // background-color: #ccc;
    margin-bottom: 20px;
    border-radius: 15px;
    border: 2px solid #005bb0;
    padding: 4px;
    position: relative;
    background: #002649;
    box-shadow: inset 0px 0px 40px 0px rgba(0, 80, 148, 0.4);
    opacity: 0.8;
  }
  // .tool-item:hover {
  //   background: #0e90ff;
  //   border: 2px solid rgba(255, 255, 255, 0.2);
  //   box-shadow: none;
  // }
  // .tool-item.active {
  //   background: #0e90ff;
  //   border: 2px solid rgba(255, 255, 255, 0.2);
  //   box-shadow: none;
  // }
  // 图层
}
.map-check {
  position: absolute;
  display: flex;
  // justify-content: space-around;
  // align-items: center;
  right: 20px;
  // top: 300px;
  bottom: 30px;
  width: 240px;
  height: 100px;
  border: 1px solid #05c1f5;
  padding: 10px;
  .map-check-item {
    width: 120px;
    height: 80px;
    border: 1px solid #ccc;
    margin-right: 10px;
    img {
      background-size: cover;
    }
  }
}
.search-tool {
  position: absolute;
  top: 20px;
  left: 20px;
  display: flex;
  .iconfont {
    font-size: 20px;
  }
}
//台站弹框
.pie-equip-panel {
  z-index: 100;
  :deep(.pie-panel-content) {
    background-color: transparent !important;
    // background: url(../../../../assets/qietu/modals-bg.png);
  }
  .point-modal-echart-box {
    position: relative;
    // border: 1px solid green;
    margin-top: 10px;
    margin-bottom: 10px;
    .echart-title {
      position: absolute;
      top: 10px;
      left: 10px;
    }
    .point-modal-echarts {
      width: 378px;
      height: 200px;
      border: 1px solid #0653a0;
    }
  }
}
// 点位弹框
.ponit-modal {
  display: flex;
  flex-direction: column;
  width: 100%;
  .point-modal-center {
    display: flex;
    // background: pink;
    color: @main-font-color;
    font-size: @main-font-size;
    padding-bottom: 10px;
    box-sizing: border-box;
    .title {
      width: 80px;
    }
    .info {
      width: calc(100% - 80px);
      white-space: pre-wrap;
      // overflow: hidden;
      // text-overflow: ellipsis;
    }
    // .info:hover {
    //   overflow: visible;
    //   white-space: pre-wrap;
    // }
    .infoImg {
      width: 100%;
      height: 300px;
    }
    .band-button {
      // width: 100px;
      // height: 50px;
      font-size: 16px;
      color: #00aadd;
      cursor: pointer;
      margin-right: 20px;
    }
  }
  .point-modal-top {
    display: flex;
    align-items: center;
    padding: 10px 0;
    box-sizing: border-box;
    .top-name {
      font-size: 18px;
      color: #fff;
      margin-right: 15px;
    }
    .top-code {
      font-size: 14px;
      color: #fff;
    }
  }

  .point-instrument {
    width: 100%;
    display: flex;
    justify-content: space-between;
    border-bottom: 1px solid #027395;
    .instrum-box {
      width: 48%;
      height: 100%;
      background: #172a3a;
      color: @main-font-color;
      padding: 10px;
      font-size: 14px;
      border: 1px solid #027395;
      border-radius: 2px;
      box-shadow: 0 0 4px #00aadd;
      text-align: center;
      cursor: pointer;
      .instrum-status {
      }
      .instrum-number {
        text-align: center;
        margin-top: 10px;
        font-size: 24px;
        font-weight: 700;
        color: #dd0000;
      }
      .inform {
        color: #00ff00;
      }
    }
    .instrum-box:hover {
      box-shadow: 0 0 6px #00aadd;
    }
  }
}
//设备弹框
.pie-mark-modal {
  position: absolute;
  right: 5%;
  top: 8%;
}
// 左侧搜索框
.leftSearch-show {
  position: absolute;
  display: flex;
  left: 0;
  top: 0;
  padding: 10px 20px;
  width: 100%;
  background-color: rgba(17, 46, 110, 0.7);
  .leftSearch-content {
    display: flex;
    align-items: center;
    // background: red;
    margin-right: 10px;
    .leftSearch-font {
      font-size: @main-font-size;
      color: @main-font-color;
      margin-right: @title-padding;
    }
    .leftSearch-station {
      width: 37px;
    }
  }
  .leftSearch-content:last-child {
    margin-right: 0;
  }
}
// 树形结构（dou）
.tree-total {
  z-index: 1;
  position: absolute;
  left: 38px;
  top: 80px;
  display: flex;
  flex-direction: column;
  justify-content: space-around;
  // border: 1px solid #05c1f5;
  padding: 10px;
  box-sizing: border-box;
  border-radius: 4px;
  background: linear-gradient(
      rgba(0, 38, 73, 0.88) 0%,
      rgba(0, 47, 91, 0.96) 100%
    )
    transparent;
  box-shadow: rgb(61 141 255 / 30%) 0px 4px 20px 0px,
    rgb(0 100 186 / 50%) 0px 0px 32px 0px inset;
  border: 1px solid;
  border-image: linear-gradient(rgb(0, 133, 192), rgb(0, 91, 176)) 1 / 1 / 0
    stretch;
  // background: url(../../../../assets/qietu/modals-bg.png);
  background-size: 100% 100%;
  // background: rgba(14, 35, 90, 0.5);
  :deep(.el-tree) {
    width: 300px;
    height: 500px;
    overflow-y: auto;
    --el-tree-node-hover-bg-color: rgba(255, 255, 255, 0.3);
  }
  :deep(.el-tree)::-webkit-scrollbar {
    display: none;
  }
  .icon-box {
    overflow: hidden;
    margin-right: -17px;
    margin-top: 4px;
    .arrow-box {
      color: #fff;
      float: right;
      // font-size: 16px;
      cursor: pointer;
    }
    .iconfont {
      font-size: 12px;
      background-color: #0e90ff;
      border-radius: 50%;
      padding: 5px;
    }
  }
}

.tree-total::before {
  content: "";
  display: block;
  /* width: 8px; */
  // border-left: 28px solid rgba(0, 0, 0, 0);
  border-top: 10px solid transparent;
  border-left: 7px solid #0173dd;
  border-bottom: 10px solid transparent;
  /* border-right: 10px solid red; */
  height: 32px;
  position: absolute;
  left: -1px;
  top: 7%;
}

.tree-total::after {
  content: "";
  display: block;
  /* width: 8px; */
  border-top: 10px solid transparent;
  border-right: 7px solid #0173dd;
  border-bottom: 10px solid transparent;
  /* border-right: 10px solid red; */
  height: 20%;
  width: 0px;
  position: absolute;
  right: 0px;
  top: 67%;
}
.tree-total1 {
  display: none;
  z-index: 1;
  position: absolute;
  left: 0px;
  top: 80px;

  .icon-box {
    overflow: hidden;
    margin-right: -17px;
    margin-top: 4px;
    .arrow-box {
      color: #fff;
      float: right;
      // font-size: 16px;
      cursor: pointer;
    }
    .iconfont {
      font-size: 12px;
      background-color: #0e90ff;
      border-radius: 50%;
      padding: 5px;
    }
  }
}
// 设备波段弹框
.panel-instrument {
  .band-box {
    width: 100%;
    height: 100%;
    display: flex;
    flex-direction: column;
    // flex-wrap: wrap;
    margin-top: 14px;
    .band-box-input {
      // height:40px;
      padding-bottom: 12px;
      span {
        color: #98dcff;
        margin-right: 20px;
      }
    }
    .band-box-item {
      height: 600px;
      display: flex;
      flex-direction: column;

      .band-item {
        width: 100%;
        height: 350px;
        border: 1px solid rgba(255, 255, 255, 0.3);
        color: #fff;
        // margin-left: 2%;
        margin-bottom: 10px;
        span {
          color: #98dcff;
          height: 30px;
          line-height: 30px;
          padding-left: 20px;
          padding-right: 20px;
          box-sizing: border-box;
          font-size: 18px;
          margin-top: 5px;
          display: flex;
          justify-content: flex-start;
          align-items: center;
          overflow: hidden;
          text-overfolw: ellipsis;
          white-space: nowrap;
        }
        .title-name {
          height: 30px;
          line-height: 30px;
          // padding-left: 20px;
          font-size: 18px;
          // margin-top: 5px;
          display: flex;
          justify-content: space-between;
          span {
            color: #98dcff;
          }
          :deep(.el-button > span) {
            color: #98dcff;
          }
        }
      }
    }
    :deep(.title) {
      display: none;
    }
    :deep(.echarts-assembly) {
      margin-top: 30px;
    }
  }
}
.legend-style {
  position: absolute;
  left: 35px;
  bottom: 10px;
}
.legend-icon {
  position: absolute;
  left: 15px;
  bottom: 100px;
  font-size: 25px;
  color: #076dcc;
}
.legend-icon-up {
  padding: 6px;
  border: 1px solid #076dcc;
  border-radius: 20px;
  position: absolute;
  right: @global-padding;
  bottom: @global-padding;
  font-size: 25px;
  color: #076dcc;
  background: url("../assets/qietu/down.png");
  background-size: 100% 100%;
  width: 28px;
  height: 28px;
}
.search-style {
  position: absolute;
  left: @global-padding;
  top: @global-padding;
}
.legend-icon:hover {
  color: #2181f0;
}

//地图设备弹框 echart图特殊样式
.equip-box {
  // background: url(../../../../assets/qietu/modals-bg.png);
  background-size: 100% 100%;
  .band-item {
    border: 0 !important;
  }
}

.panel-instrument-big {
  z-index: 3;
  display: flex;
  justify-content: center;
  align-items: center;
  :deep(.pie-panel-content) {
    z-index: 200 !important;
  }
  .big-echarts {
    height: 100%;
    width: 100%;
    display: flex;
    // justify-content: space-around;
    // align-items: center;
    flex-direction: column;
    .sample-name {
      height: 40%;
      display: flex;
      justify-content: space-around;
      align-items: center;

      span {
        width: 30%;
        height: 100%;
        color: white;
        display: flex;
        justify-content: center;
        align-items: center;
      }
      .echarts-img {
        //  width:30%;
        height: 100%;
      }
    }
    .select-style-name {
      width: 100px;
    }
    .select-style {
      margin-top: 10px;
      width: 410px !important;
      // height: 40px;
      :deep(.el-select) {
        margin-top: 10px;
        width: 337px !important;
        // height: 40px;
      }
    }

    .sample-upload {
      margin-top: 30px;
      height: 40%;
      display: flex;
      justify-content: flex-end;
    }
    :deep(.el-button) {
      --el-button-text-color: #98dcff;
    }
  }
}
.isNetwork {
  position: absolute;
  // background: url(../../../../assets/qietu/modals-bg.png);
  background-size: 100% 100%;
  display: flex;
  right: 80px;
  top: 16%;
}
.facility-table {
  // padding: 40px 0;
  position: absolute;
  display: flex;
  left: 20px;
  top: 30px;
}
// 模型dom大小
#threeContainer {
  width: 1500px;
  height: 800px;
}
.three-info {
  width: 200px;
  height: 100px;
  position: absolute;
  left: 0;
  top: 0;
  color: #fff;
  background: hsla(209, 100%, 14%, 0.45);
  border: 1px solid #00aadd;
  border-radius: 4px;
  padding: 10px;
}
// :deep(.el-input__wrapper) {
//   width: 250px;
//   border-radius: 2px 0px 0px 2px;
// }
// :deep(.el-button > span) {
//   color: #fff;
// }
:deep(.el-button) {
  width: 50px;
  height: 36px;
  border-radius: 0px 2px 2px 0px;
  background-color: #00aadd;
}
:deep(.pie-panel .pie-panel-content) {
  z-index: 0;
}
:deep(.confirm) {
  width: 4rem;
  border-radius: 4px;
  height: 2rem;
  margin-right: 10px;
}
:deep(.el-button) {
  background-color: transparent;
  border: 1px solid #00aadd;
}

.home-content {
  height: 100%;
  overflow-y: auto;
  margin-left: 20px;
  width: 100%;
  border: 1px solid @global-header-bbg;
  // padding: 20px;
  box-sizing: border-box;
}

.select-btn {
  position: absolute;
  top: 90px;
  left: 40px;
  // background: url("../../../../assets/image/select.png");
  background-size: 100% 100%;
  width: 325px;
  height: 40px;
  :deep(.el-input__wrapper) {
    border: none;
  }
  .search-img {
    height: 10px;
  }
  :deep(.el-input-group__prepend) {
    padding: 0 5px;
    background: transparent;
  }
  :deep(.el-input) {
    border: 1px solid #005bb0;
    background: rgba(24, 77, 126, 0.8);
    --el-input-focus-border-color: transparent;
    --el-input-hover-border-color: transparent;
  }
}
//panel背景
:deep(.pie-panel .pie-panel-content) {
  // background: url(../../../../assets/qietu/modals-bg.png);
  background-size: 100% 100%;
  // background: none;
  //backGroundColor="'rgba(26, 53, 74, 0.9)'"
}

:deep(.el-input__wrapper) {
  width: 10rem;
}
:deep(.el-tabs__nav-wrap::after) {
  position: static !important;
}
:deep(.el-tabs__item) {
  color: #fff;
}
:deep(.pie-panel .operate-item) {
  overflow-y: scroll;
  margin-bottom: 0;
  max-height: 850px;
  padding: 15px;
}
:deep(.el-table tr) {
  cursor: pointer;
}
:deep(.el-select-dropdown__item) {
  padding: 0 @global-small-padding;
}
:deep(.select-box) {
  position: absolute;
  left: 20px;
  top: 20px;
}
.net-box {
  position: absolute;
  right: 75px;
  top: @global-padding;
  padding: 10px;
  background: linear-gradient(
      rgba(0, 38, 73, 0.88) 0%,
      rgba(0, 47, 91, 0.96) 100%
    )
    transparent;
  box-shadow: rgb(61 141 255 / 30%) 0px 4px 20px 0px,
    rgb(0 100 186 / 50%) 0px 0px 32px 0px inset;
  border: 1px solid;
  border-image: linear-gradient(rgb(0, 133, 192), rgb(0, 91, 176)) 1 / 1 / 0
    stretch;
  color: @main-font-color;
  width: 420px;
  .title-line {
    display: flex;
    font-size: 16px;
    position: relative;

    .iconfont {
      font-size: 24px;
      margin-right: 20px;
    }

    .cancel-icon {
      position: absolute;
      right: -20px;
      cursor: pointer;
    }
  }

  .echarts-info-title {
    position: relative;

    .station-view {
      position: absolute;
      top: 10px;
      left: 10px;
    }

    .device-view {
      position: absolute;
      top: 10px;
      left: 207px;
    }
  }

  .echarts-info {
    display: flex;
    border: 1px solid rgb(6, 83, 160);
    // background-color: rgba(2, 12, 20,0.4);
    margin: 10px 0;

    .echarts-item {
      width: 200px;
      height: 190px;
      // border-right: 1px solid @border-color-selected;
    }

    .left-item {
      border-right: 1px solid rgb(6, 83, 160);
    }
  }

  .equipment-list {
    display: flex;
    flex-wrap: wrap;
    border: 1px solid rgb(6, 83, 160);
    padding: 10px;

    .item-info {
      display: flex;
      flex-direction: column;
      align-items: center;
      justify-content: center;
      width: 125.5px;
      height: 70px;
      background-color: #1f6c94;
      margin-right: 10px;
      margin-top: 10px;
      font-size: 16px;
      color: #fff;

      .num {
        color: red;
        font-weight: bold;
      }
    }

    .ismargin {
      margin-right: 0;
    }
  }
  .histogram-group {
    position: relative;
    margin-top: 10px;
    border: 1px solid rgb(6, 83, 160);

    .histogram-title {
      position: absolute;
      top: 20px;
      left: 14px;
    }
    .histogram {
      width: 390px;
      height: 400px;
      max-width: 100%;
    }
  }
}
.opeartion-item {
  color: #005bb0;
  // font-size: 20px;
  cursor: pointer;
}
:deep(.editBtn-table[data-v-b219f9c4]) {
  margin-right: 0 !important;
}

.modal-wrapper {
  display: flex;
  width: 100%;
  justify-content: center;
  flex-direction: column;
  // max-height: 800px;
  // overflow-y: scroll;
  padding: @global-padding;
  // flex-wrap: wrap;
  .essential-information {
    width: 100%;
    border: 1px solid #2181f0;
    margin-bottom: 20px;
    margin-top: 20px;
    padding-top: @global-padding;
    padding-left: @global-padding;
    padding-right: @global-padding;
    box-sizing: border-box;
    display: flex;
    flex-wrap: wrap;
    position: relative;
    .information-title {
      background: linear-gradient(rgba(0, 38, 73) 0%, rgba(0, 47, 91) 100%)
        transparent;
      width: 100px;
      height: 30px;
      position: absolute;
      left: 40px;
      top: -16px;
      z-index: 3;
      display: flex;
      align-items: center;
      justify-content: center;
      font-size: 16px;
    }
    .info-item {
      display: flex;
      flex-wrap: wrap;
      align-items: center;
      width: 50%;
      margin-bottom: @global-margin;
      .title {
        margin-right: @global-small-padding;
      }
    }
  }
  .table-content {
    margin-bottom: @global-margin;
  }
}
.monitor-img {
  margin-left: @global-small-padding;
  color: @main-active-color;
}
.quipment-details {
  z-index: 9;
}
.img-content {
  max-height: 780px;
  overflow-y: scroll;
  padding: 0 @global-padding;
  .detail-img {
    width: 100%;
    height: 240px;
    margin-bottom: @global-margin;
  }
}
//点击台站设备详情
.modal-margin {
  margin-bottom: 10px;
}
.modal-content {
  display: flex;
  justify-content: center;
  align-items: center;
  margin-bottom: 20px;
  width: 100%;

  .modal-label-name {
    display: inline-block;
    width: 150px;
    text-align: left;
    color: @main-font-color;
    font-size: @main-font-size;
    padding-right: @title-padding;
  }
  .equdetails {
    display: inline-block;
    width: 299px;
    text-align: left;
    color: @main-font-color;
    font-size: @main-font-size;
    // padding-right: @title-padding;
  }
  .edit-select {
    width: 300px;
  }
  .el-input,
  .el-textarea {
    width: 300px;
    background: transparent;
    border-radius: 5px;
    border: 1px solid @global-border-color;
    color: @main-font-color;
  }
  .el-input {
    height: 40px;
  }
  :deep(.el-textarea__inner) {
    background: transparent;
    color: @main-font-color;
  }
  :deep(.el-cascader) {
    width: 300px;
  }
}
.station-top-item {
  display: flex;
  justify-content: flex-start;
  align-items: center;
  margin-bottom: 10px;
  .station-title {
    color: @main-font-color;
    padding-right: @title-padding;
  }
  .el-input {
    width: 220px;
    color: @main-font-color;
    background: transparent;
    border-bottom-right-radius: 5px;
  }
  :deep(.el-input__wrapper) {
    width: 220px;
    flex-grow: 0;
  }
}
.status-failure {
  .iconfont {
    font-size: 24px;
    color: rgba(223, 39, 39, 0.9);
    animation: iconStylesFailure 1s infinite alternate;
  }
  @keyframes iconStylesFailure {
    0% {
      color: rgba(223, 39, 39, 0.9);
    }
    50% {
      color: rgba(223, 39, 39, 0.5);
    }
    100% {
      color: rgba(223, 39, 39, 0.1);
    }
  }
}
.status-available {
  .iconfont {
    font-size: 24px;
    color: rgba(73, 251, 24, 0.9);
    animation: iconStyleAvailable 1s infinite alternate;
  }
  @keyframes iconStyleAvailable {
    0% {
      color: rgba(73, 251, 24, 0.9);
    }
    50% {
      color: rgba(73, 251, 24, 0.5);
    }
    100% {
      color: rgba(73, 251, 24, 0.1);
    }
  }
}
.status-warning {
  .iconfont {
    font-size: 24px;
    color: rgba(230, 227, 60, 0.952);
    animation: iconStyleWarning 1s infinite alternate;
  }
  @keyframes iconStyleWarning {
    0% {
      color: rgba(230, 227, 60, 0.952);
    }
    50% {
      color: rgba(156, 154, 42, 0.952);
    }
    100% {
      color: rgba(100, 99, 26, 0.952);
    }
  }
}
.status-unavailable {
  color: rgba(134, 129, 129, 0.9);
}
.lookBtn-table {
  margin-right: 15px;
  color: #00aeff;
  cursor: pointer;
}
</style>
