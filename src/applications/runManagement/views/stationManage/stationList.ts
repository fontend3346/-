...
export const useHandler = () => {
  ...
  const addPoint = () => {
    ...
    http.stationsInfoManage.getStations(params).then((res: any) => {
      ...
      if (res.code == 0) {
        ...
        baseMapObj.addStationPoints(newArr, state.layers);
  ...
  const mousewheelEvent = () => {
    ...
    pieTimer = setTimeout(() => {
      let getBounds = baseMapObj.globe.getViewBounds()
      state.mousewheelBounds = {
        coordinates: [[[getBounds.left, getBounds.top], [getBounds.right, getBounds.top], [getBounds.right, getBounds.bottom], [getBounds.left, getBounds.bottom], [getBounds.left, getBounds.top]]],
        type: "Polygon"
      }
      getList()
      addPoint()//加载所有点
  ...
  //图层点击事件监听(台站点击事件)
  const layerClicks = () => {
    baseMapObj.layerClick((layer: any, obj: any) => {
      let name = layer.getName();
      if ("searchList" == name) {
        update(obj);
      }
      baseMapObj.flyTo([obj.longitude, obj.latitude], 6);
    });
  };
  // 销毁
  onUnmounted(() => {
  ...
  const checkMapFn = (source: any) => {
    let bottomLayerGroup = baseMapObj.globe.getLayer("底图图层");
    if (!bottomLayerGroup) {
      bottomLayerGroup = new PIE.GlobeGroupLayer();
      bottomLayerGroup.setName("底图图层");
      baseMapObj.globe.addLayer(bottomLayerGroup, true);
    }
    ...
    baseMapObj.globe.refresh(true, false);
