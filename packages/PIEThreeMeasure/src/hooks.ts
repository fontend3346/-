...
// 操作
export const useHandler = (props: any, emit: Emits) => {
  ...
  const createHalfBall = () => {
    ...
    const material = textureLoader(cement2);
  ...
  // 创建地板
  const createFloor = () => {
    let geometry = new THREE.BoxGeometry(1500, 1, 1500);
    const material1 = new THREE.MeshLambertMaterial({ color: 0x64B5D6 });
    const material2 = textureLoader(cement3);
    let materialArry: any = [material1, material1, material2, material1, material1, material1];
    const mesh = new THREE.Mesh(geometry, materialArry);
    mesh.receiveShadow = true;
    mesh.name = '土地';
    scene.add(mesh);

    let geometryGrass = new THREE.BoxGeometry(5000, 1, 5000);
    const materialGrass = textureLoader(grass);
    let materialArryGrass: any = [materialGrass, materialGrass, materialGrass, materialGrass, materialGrass, materialGrass];
    const meshGrass = new THREE.Mesh(geometryGrass, materialArryGrass);
    meshGrass.name = '草地';
    meshGrass.position.set(0, -1, 0);
    scene.add(meshGrass);
  }
  // 创建单个围栏
  const createWall = (obj: any) => {
    let geometry = new THREE.BoxGeometry(obj.size.width, obj.size.height, obj.size.length);
    loader = new THREE.TextureLoader();
    let texture = loader.load(obj.skin);
    texture.repeat.set(1, 1);
    let material: any = new THREE.MeshLambertMaterial({ map: texture, color: 0xffffff });
    let material2 = new THREE.MeshLambertMaterial({ color: 0xffffff });
    material.transparent = true
    // material.opacity = 0.8;
    const mesh = new THREE.Mesh(geometry, [material, material, material, material, material, material]);
  ...
  const createCylinder = () => {
    ...
    const material = textureLoader(cy1);
  ...
  const createTripod = () => {
    state.tripodData.forEach((item: any) => {
      ...
      const material = textureLoader(cement4);
      const cylinder = new THREE.Mesh(geometry, material);
    ...
    const mesh1 = new THREE.Mesh(geometry1, [material2, material2, material2, material2, material1, material1]);
  ...
  const createDevice = () => {
    ...
    const material = new THREE.MeshLambertMaterial({ color: 0xC9BFAC, side: THREE.DoubleSide, });
    const cylinder = new THREE.Mesh(geometry, material);
    ...
    const material2 = textureLoader(cy1);
    ...
    state.deviceData.forEach((item: any) => {
      let geometry2 = new THREE.BoxGeometry(item.size.width, item.size.height, item.size.length);
  ...
  const createPole = () => {
    ...
    const material5 = textureLoader(tiepi1);
    // const material2 = new THREE.MeshBasicMaterial({ color: 0xD9D0BE });
    ...
    const material = textureLoader(tiepi1);
    // const material2 = new THREE.MeshBasicMaterial({ color: 0xD9D0BE });
    const cylinder = new THREE.Mesh(geometry, material);
    ...
    state.conectData.forEach((item: any) => {
      const box = new THREE.BoxGeometry(item.size.width, item.size.height, item.size.length);
      let materialBox: any = null;
      let material = textureLoader(item.skin);;
      let material2 = null;
      if (item.id == 1) { // 如果是盒子门
        material2 = textureLoader(item.skin2);
        materialBox = [material2, material2, material2, material2, material, material2]
      } else {
        materialBox = [material, material, material, material, material, material]
      }
  ...
  const createInnerDevice = () => {
    ...
    const material3: any = textureLoader(inner2);
    const material4: any = textureLoader(inner3, 6, 1);
    // const material2 = new THREE.MeshBasicMaterial({ color: 0xD9D0BE });
    const cylinder3: any = new THREE.Mesh(geometry3, [material4, material3, material4]);
    cylinder3.position.set(-340, 25, 420);
    cylinder3.name = '仪器in';
    cylinder3.info = {
      uuid: '仪器'
    }
    cylinder3.hover = function (o: any) { // 鼠标移入设备
      moveShow.value = true;
  ...
  const textureLoader = (imgUrl: string, x?, y?) => {
    ...
    material = new THREE.MeshLambertMaterial({ map: texture, side: THREE.DoubleSide, color: 0xCED0D1 });
    return material;
  }
    ...
    if (intersects.length !== 0 && intersects[0].object.type === 'Mesh') {
      ...
      switch (intersects[0].object.name) {
        ...
        case '半球盖子2':
          ...
          break;
        case '仪器in':
          intersects[0].object.position.set(-340, 60, 420);
          intersects[0].object.name = '仪器out';
          break;
        case '仪器out':
          intersects[0].object.position.set(-340, 25, 420);
  ...
  const outlineObj = (selectedObjects: any) => {
    ...
    // 物体边缘发光通道
    outlinePass = new OutlinePass(new THREE.Vector2(threeRef.value.clientWidth, threeRef.value.clientHeight), scene, camera, selectedObjects)
    ...
    effectFXAA.uniforms.resolution.value.set(1 / threeRef.value.clientWidth, 1 / threeRef.value.clientHeight)
  ...
  const createSky = () => {
    const uniforms = {
      topColor: { value: new THREE.Color(0xFBF3DE) },
    ...
    const skyGeo: any = new THREE.SphereGeometry(4000, 32, 15);
    const skyMat: any = new THREE.ShaderMaterial({
      uniforms: uniforms,
  ...
  // 测点模型
  watch(() => props.measureShow, () => {
    if (props.measureShow) {
      nextTick(() => {
        initModel();
      });
    }
