<template>
  <div class="left-dialog">
    ...
    <div class="table-top">
      <div class="table-content" v-if="activeIndex == 0">
    ...
    <div class="main-table" v-if="activeIndex == 0">
      ...
      >
        ...
        <template v-slot:receiveStatus="receiveStatus">
          <div v-if="receiveStatus.row.receiveStatus == 0" class="status-open">
            未开启
          </div>
          <div v-else class="status-close">已开启</div>
        </template>
        <template #action="scope">
          <span class="activeBtn-table" @click="activeOpen(scope.row)"
            >关闭</span
          >
          <span
            class="activeBtn-table"
            v-if="scope.row.receiveStatus == 0"
            @click.stop="confirmStartUse(scope.row)"
          >
            <span>开启</span>
          </span>
          ...
          <!-- <span
            class="activeBtn-table"
            v-if="scope.row.receiveStatus != 0"
            @click="waveformFun(scope.row)"
            >波形</span
          > -->
          <span
            class="activeBtn-table"
            v-if="scope.row.receiveStatus != 0"
            @click.stop="logFun(scope.row)"
            >查看</span
          >
        </template>
      </PIETable>
